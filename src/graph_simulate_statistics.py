...
from graph_characteristic import (
    ...
    calculate_maxsize_independed_set,
...
def gen_skewnormal_points(
    num_points: int, alpha: float = SKEW_ALPHA_0
...
def gen_laplace_points(
    num_points: int, alpha: float = LAPLACE_ALPHA_0, beta: float = LAPLACE_BETA_0
) -> np.ndarray[np.float64]:
...
def gen_weibull_points(
    num_points: int, k: float = WEIBULL_K_0, lambd: float = WEIBULL_LAMBDA_0
) -> np.ndarray[np.float64]:
...
def gen_gamma_points(
    num_points: int, k: float = GAMMA_K_0, lambd: float = GAMMA_LAMBDA_0
) -> np.ndarray[np.float64]:
...
def simulate_graph_statistics(
        num_samples: int = 420,
        vector_size: int = 42,
        distribution_type: str = "gamma_weibull",
        gamma_k: float = 0.5,
        gamma_lambda: float = 1 / np.sqrt(2),
        weibull_k: float = 0.5,
        weibull_lambda: float = 1 / np.sqrt(10),
        skew_alpha: float = 1,
        laplace_alpha: float = 0,
        laplace_beta: float = 1 / np.sqrt(2),
        T_knn_foo: Callable[[Graph], int] = calculate_number_triangle,
        knn_num_neighbours: int = 42,
        T_dist_foo: Callable[[Graph], int] = calculate_maxsize_independed_set,
        dist_max_dist: float = 4.2,
        verbose: bool = False
) -> dict:
    """
    ...
    Параметры:
    - num_samples:          количество реализаций характеристики
    - vector_size:          размерность случайного вектора
    - distribution_type:    "gamma_weibull" или "laplace_skewnormal" - тип используемых распределений
    - gamma_k:              параметр k гамма-распределения
    - gamma_lambda:         параметр lambda гамма-распределения
    - weibull_k:            параметр k распределения Вейбулла
    - weibull_lambda:       параметр lambda распределения Вейбулла
    - skew_alpha:           параметр alpha косого нормального распределения
    - laplace_alpha:        параметр alpha распределения Лапласа
    - laplace_beta:         параметр beta распределения Лапласа
    - T_knn_foo:            функция для KNN графа
    - knn_num_neighbours:   количество соседей KNN графа
    - T_dist_foo:           функция для Distance графа
    - dist_max_dist:        максимальная длина соединения Distance графа
    - verbose:              рисовать ли гистограммы
    """
    ...
    if distribution_type == "gamma_weibull":
        def gen_dist1(n):
            return np.random.gamma(gamma_k, gamma_lambda, n)

        def gen_dist2(n):
            return weibull_lambda * np.random.weibull(weibull_k, n)
        ...
        def gen_dist1(n):
            return skewnorm.rvs(skew_alpha, size=n)

        def gen_dist2(n):
            return np.random.laplace(loc=laplace_alpha, scale=laplace_beta, size=n)
    ...
    if verbose:
        ...
        plt.subplot(1, 2, 1)
        bins = np.arange(
            min(T_knn_dist1_list + T_knn_dist2_list), max(T_knn_dist1_list + T_knn_dist2_list) + 1, 1
        )
        ...
        plt.subplot(1, 2, 2)
        bins = np.arange(
            min(T_dist_dist1_list + T_dist_dist2_list), max(T_dist_dist1_list + T_dist_dist2_list) + 1, 1
        )
...
def generate_dataset(
    ...
    distribution_1_characteristics = pd.DataFrame(
        distribution_1_characteristics,
        columns=characteristics_names)
    distribution_2_characteristics = pd.DataFrame(
        distribution_2_characteristics,
        columns=characteristics_names)
    ...
    df = pd.concat([distribution_1_characteristics,
                   distribution_2_characteristics], ignore_index=True)
...
def plot_distribution_parameter_combinations(
        laplace_alphas: float,
        laplace_betas: float,
        skew_norm_alphas: float,
        num_samples: int = 200,
        vector_size: int = 40,
        knn_num_neighbours: int = 3,
        dist_max_dist: float = 1
) -> None:
    """
    ...
    for alpha in laplace_alphas:
        for beta in laplace_betas:
            for alpha_skew in skew_norm_alphas:
                ...
                plt.hist(
                    result["T_knn_skew_normal_list"],
                    bins=bins,
                    align="mid",
                    alpha=0.5,
                    label="Косое нормальное")
                plt.hist(
                    result["T_knn_laplace_list"],
                    bins=bins,
                    align="mid",
                    alpha=0.5,
                    label="Лаплас")
                ...
                plt.hist(
                    result["T_dist_skew_normal_list"],
                    bins=bins,
                    align="mid",
                    alpha=0.5,
                    label="Косое нормальное")
                plt.hist(
                    result["T_dist_laplace_list"],
                    bins=bins,
                    align="mid",
                    alpha=0.5,
                    label="Лаплас")
...
def plot_graphs_parameter_combinations(
        sizes: np.ndarray[float] = [40],
        neighbours: np.ndarray[float] = [3],
        dists: np.ndarray[float] = [1],
        graph_type: str = "knn"
) -> None:
    """
    - sizes:                сетка размеров
    - neighbours:           сетка кол-ва соседей
    - dists:                сетка длин связи
    - graph_type:           тип графа, "knn" иди "dist"
    """
    plot_counter = 0
    ...
    for n in sizes:
        for neigh in neighbours:
            for dist in dists:
                ...
                bins = np.arange(
                    min(result[f"T_{graph_type}_skew_normal_list"] +
                        result[f"T_{graph_type}_laplace_list"]),
                    max(result[f"T_{graph_type}_skew_normal_list"] +
                        result[f"T_{graph_type}_laplace_list"]) + 1,
                    1
                )

                plt.hist(
                    result[f"T_{graph_type}_skew_normal_list"],
                    bins=bins,
                    align="mid",
                    alpha=0.5,
                    label="Skew normal"
